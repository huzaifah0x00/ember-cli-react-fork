 ...
 */
export default function YieldWrapper(props: { nodes: Node[] }) {
  ...
  useEffect(() => {
    ...
    if (!el.current) throw new Error('YieldWrapper: el is not defined');
    if (!el.current.parentNode) throw new Error('YieldWrapper: el.parentNode is not defined');
